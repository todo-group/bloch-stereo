...
import { blochVectorsForDensityMatrix, blochVectorsForState, correlationMatrix, correlationMatrixFromDensityMatrix } from "./density";
import { teleportationQasm } from "../../presets/teleportation";
import type { ExecutionState } from "../types";

describe("statevector execution", () => {
  it("creates Bell-state connected correlations without normalizing reduced Bloch vectors", () => {
...
`);
  ...
  it("returns zero connected correlations for product states", () => {
    const circuit = parseQasm2(`OPENQASM 2.0;
include "qelib1.inc";
qreg q[2];
creg c[2];
`);
    ...
    const final = lastSnapshot(executeCircuit(circuit));
    const correlations = correlationMatrix(final!.statevector, circuit.numQubits, 0, 1);
    correlations.forEach((row) => row.forEach((value) => expect(value).toBeCloseTo(0)));
  });
...
describe("density-matrix execution", () => {
  it("matches Bell-state reduced densities and connected correlations", () => {
    const circuit = parseQasm2(`OPENQASM 2.0;
include "qelib1.inc";
qreg q[2];
creg c[2];
...
`);
    ...
    const final = lastSnapshot(executeCircuit(circuit, { backend: "density-matrix" }));
    expect(final?.densityMatrix).toBeDefined();
    const vectors = blochVectorsForDensityMatrix(final!.densityMatrix!, circuit.numQubits);
    expect(vectors[0].purity).toBeCloseTo(0.5);
    expect(vectors[1].purity).toBeCloseTo(0.5);

    const correlations = correlationMatrixFromDensityMatrix(final!.densityMatrix!, circuit.numQubits, 0, 1);
    expect(correlations[0][0]).toBeCloseTo(1);
    expect(correlations[1][1]).toBeCloseTo(-1);
    expect(correlations[2][2]).toBeCloseTo(1);
  });

  it("collapses selected measurements and updates classical bits", () => {
    const circuit = parseQasm2(`OPENQASM 2.0;
include "qelib1.inc";
qreg q[1];
creg c[1];
...
`);
    ...
    const final = lastSnapshot(executeCircuit(circuit, { backend: "density-matrix", forcedMeasurements: [1] }));
    expect(final?.classicalBits).toEqual([1]);
    expect(final?.measurementLog[0]).toMatchObject({ qubit: 0, clbit: 0, value: 1 });
    expect(final?.measurementLog[0].probability).toBeCloseTo(0.5);

    const vector = blochVectorsForDensityMatrix(final!.densityMatrix!, circuit.numQubits)[0];
    expect(vector.z).toBeCloseTo(-1);
  });

  it("returns zero connected correlations for product density matrices", () => {
    const circuit = parseQasm2(`OPENQASM 2.0;
include "qelib1.inc";
qreg q[2];
creg c[2];
...
`);
  ...
  it("applies depolarizing channels to produce product mixed states", () => {
    const circuit = parseQasm2(`OPENQASM 2.0;
include "qelib1.inc";
qreg q[2];
creg c[2];
...
`);
  ...
  it("keeps teleportation corrections aligned with selected measurement outcomes", () => {
    const circuit = parseQasm2(teleportationQasm);
    const final = lastSnapshot(executeCircuit(circuit, { backend: "density-matrix", forcedMeasurements: [1, 1] }));
    const target = blochVectorsForDensityMatrix(final!.densityMatrix!, circuit.numQubits)[2];
    expect(target.x).toBeCloseTo(1);
    expect(target.y).toBeCloseTo(0);
    expect(target.z).toBeCloseTo(0);
  });
